...
urlpatterns = patterns('',
	...
	url(r'^Factura/(?P<id_Cliente>\d+)?\+?(?P<RFC_Cliente>\w+)?\+?(?P<Moroso>\w+)?\+?(?P<Dolar>[\w.]+)?','principal.views.v_Factura'),
    ...
    url(r'^Reporte_Facturas/(?P<FechaInicio>[\w-]+)?\+?(?P<FechaFinal>[\w-]+)?\+?(?P<Status>\w+)?', 'principal.views.v_Reporte_Facturas'),
    url(r'^Reporte_Estado/(?P<id_Cliente>\d+)$', 'principal.views.v_Reporte_Estado'),
    url(r'^Seleccionar_ClienteReporte/(?P<Nombre_Cliente>\w+)?\+?(?P<RFC_Cliente>\w+)?$','principal.views.v_Seleccionar_ClienteReporte'),
    url(r'^Generar_Factura/(?P<datos>[\w,.$]+)$', 'principal.views.v_Generar_Factura'),
    url(r'^Dolar/$', 'principal.views.v_Dolar'),
